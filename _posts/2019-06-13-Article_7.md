...
![https://upload.wikimedia.org/wikipedia/commons/thumb/d/db/EllipticCurveCatalog.svg/1024px-EllipticCurveCatalog.svg.png](./article_7/media/image1.png)[width="4.340908792650919in"
height="4.192820428696413in"}
그림 8 타원곡선 $$[[y}^[2}}=[[x}^[3}}+ax+b$$의 $$a$$와 $$b$$의 값에 따른 plot. 각 plot의 가로축은 $$x$$축이고 세로축은 $$y$$축이며 각각의 범위는 -3부터 3까지이다. (출처: Wikipedia)
...
암호화를 수행하기 전에, 먼저 사용할 타원곡선 $$S$$와 그 위의 한 점 $$G$$를 약속 해 둔다. 이 때 $$G$$는 "generator"라 불리는 특수한 점이다[^5]. $$G$$를 사용하여 어떤 정수 값 $$x$$를 암호화 하는 연산을 $$[[\left[ x \right]}_[G}}$$라 표현하겠다. 암호화 결과를 $$X$$라 할때, 암호화 결과는 다음과 같다: 
$$$$
...
정의 에 의해, 두 정수 $$a$$와 $$b$$에 대해 암호화 연산 $$[[\left[ \cdot \right]}_[G}}$$는 linear 연산이다. 즉, 다음과 같은 additivity와 scalability (homogeneity)를 갖는다: 
$$$$
[[\left[ a+b \right]}_[G}}=[[\left[ a
\right]}_[G}}+[[\left[ b \right]}_[G}},
$$$$

$$$$
a[[\left[ b \right]}_[G}}=[[\left[ ab \right]}_[G}}.
$$$$
...
& \Leftrightarrow e[[\left( G,H \right)}^[xy}}e[[\left( G,H
\right)}^[z}}=e[[\left( G,H \right)}^[w}} \\

& \Leftrightarrow e[[\left( G,H \right)}^[xy+}}^[z}=e[[\left( G,H
\right)}^[w}} \\

& \Leftrightarrow e\left( \left( xy+z \right)G,H \right)=e\left(
wG,H \right). \\
...
$$$$
e\left( xG,yH \right)=:[[\left[ x \right]}_[G}}\cdot
[[\left[ y \right]}_[H}}.
$$$$
...
$$$$
[[\left[ xy \right]}_[G\cdot H}}:=[[\left[ x
\right]}_[G}}\cdot [[\left[ y \right]}_[H}}=e\left( xG,yH
\right).
$$$$


새롭게 정의된 연산 $$[[\left[ \cdot \right]}_[G\cdot H}}$$의 bilinearity를 다음과 같이 표기한다:
$$$$
\begin[align}

& e\left( xG,H \right)e\left( yG,H \right)=[[\left[ x
\right]}_[G\cdot H}}+[[\left[ y \right]}_[G\cdot H}} \\

& =[[\left[ x+y \right]}_[G\cdot H}} \\
